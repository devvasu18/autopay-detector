    ...
    override fun onReceive(context: Context, intent: Intent) {
        if (intent.action == Telephony.Sms.Intents.SMS_RECEIVED_ACTION) {
            ...
            for (message in messages) {
                ...
                processSingleSMS(context, db, smsId, address, body, date)
            }
        }
    }

    companion object {
        fun processSingleSMS(context: Context, db: SQLiteDatabase, smsId: String, address: String, body: String, date: Long) {
            try {
                // Save raw SMS
                ...
                if (FinanceParser.isFinancialSMS(address, body)) {
                    ...
                    if (parsed != null) {
                        ...
                        if (FinanceCoreModule.instance != null) {
        ...
        private fun speakTransactionInBackground(context: Context, db: SQLiteDatabase, tx: FinanceParser.ParsedSMS) {
            ...
            if (isCancelled || isCreated) {
                ...
                when (voiceLang) {
                    "hi" -> {
                        text = if (isUpcoming) {
                            "याਦ ਦਿਲਾਓ: $merchant के लिए $amount रुपये का आगामी भुगतान"
